...
public class TPS_Controller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Determine if character is in an airborne state (flying, hovering, or falling)
        bool isFlying = animator.GetBool("IsFlying");
        bool isFalling = animator.GetBool("IsFalling");
        bool isHoveringState = animator.GetBool("IsHovering");
        bool isAirborne = !grounded && (isFlying || isFalling || isHoveringState);
        
        // Handle controller resizing based on airborne state
        if (isAirborne)
        {
            // Set flight mode controller dimensions only once when entering airborne state
            if (!isInFlightMode)
            {
                ...
                {
                    controller.center = new Vector3(0, 0.1f, 0); // Fallback center for small collider
                }
                isInFlightMode = true;
                isLanding = false;
                animator.ResetTrigger("Landing"); // Reset trigger on entering airborne state
                Debug.Log("Entered airborne mode - controller resized");
            }

            // Check for landing - use raycast to detect ground proximity
            RaycastHit hit;
            float landingCheckDistance = 1.2f; // Increased distance for better detection
            
            // Always check for landing when airborne and moving downward
            bool movingDownward = velocity.y < 0 || (isFlying && transform.forward.y < 0);
            
            if (!isLanding && Physics.Raycast(controller.transform.position, Vector3.down, out hit, landingCheckDistance, groundLayerMask))
            {
                Debug.DrawRay(controller.transform.position, Vector3.down * landingCheckDistance, Color.red);
                ...
                // Trigger landing when close enough to ground and nearly upright (or just falling normally)
                float distanceToGround = hit.distance;
                bool closeToGround = distanceToGround < 0.8f;
                bool isNearlyUpright = angle < 15f; // More lenient angle for falling
                
                if (closeToGround && (isNearlyUpright || isFalling))
                {
            ...
            {
                Debug.DrawRay(controller.transform.position, Vector3.down * landingCheckDistance, Color.green);
            }
        ...
        {
            // Grounded - reset controller dimensions if we were in flight mode
            if (isInFlightMode)
            {
                controller.height = defaultHeight;
                controller.center = defaultCenter;
                isInFlightMode = false;
                Debug.Log("Exited airborne mode - controller restored");
            }
            
            // Reset landing state after landing completes
            if (isLanding && grounded)
            {
                isLanding = false;
                animator.ResetTrigger("Landing"); // Reset trigger after landing is complete
    ...
    void FixedUpdate()
    {
        ...
        if (grounded)
        {
            ...
            // Note: Landing trigger is reset in Update() after landing completes
        }
